...
var StorageManager = {
	...
	initialize : function(idConf){
		StorageManager.idConf = idConf;
	...
	set : function(key,dataContainer){
		if(this.commandStore !== undefined){
			this.commandStore[key+StorageManager.idConf] = dataContainer;
		}else{
			$.jStorage.set(key+StorageManager.idConf,JSON.stringify(dataContainer));
		}
	},
	get : function(key){
		if(this.commandStore !== undefined){
			return this.commandStore[key+StorageManager.idConf];
		}else{
			return JSON.parse($.jStorage.get(key+StorageManager.idConf));
		}
	},
