...
var ViewAdapter = {
	...
	addswitchButton : function (){
		...
		switchViewBtn = ViewAdapter.Text.appendButton(ViewAdapter.currentPage.find(".content"),'javascript:void(0)',btnlabel,{tiny:true,theme:"b",prepend:true, align : "right",margin: "20px"}) ;
	...
	changeMode : function(){
		...
		$.each(ViewAdapter.commands,function(i,commandItem){
			...
			if(JSONdata != null){
				...
				var ajaxData   = currentCommand.getQuery({conferenceUri : ViewAdapter.conference.baseUri, uri : ViewAdapter.uri,datasource : currentDatasource, name : name, conference : ViewAdapter.conference});
				AjaxLoader.executeCommand({datasource : currentDatasource, command : currentCommand,data : ajaxData, currentUri : ViewAdapter.uri, contentEl :  ViewAdapter.currentPage.find("#"+commandItem.name), name : name, conference : ViewAdapter.conference});
			}
		});
