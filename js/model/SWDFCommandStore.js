...
\
                    SELECT DISTINCT *  WHERE {\
					   ?eventUri iCal:dtStart ?dtStart. \
					   ?eventUri iCal:dtEnd   ?dtEnd.\
                    ...
                    } ORDER BY ?dtStart ?dtEnd ?locationUri';
			...
			if(parameters.JSONdata != null){
				...
				if(_.size(eventInfo) > 0 ){
					if(ViewAdapter.mode == "text"){
						...
						var eventStartICS  = eventInfo.eventStart.replace("+05:00","").replace(/-/g,"").replace(/:/g,""); 	
						var eventEndICS  = eventInfo.eventEnd.replace("+05:00","").replace(/-/g,"").replace(/:/g,""); 
